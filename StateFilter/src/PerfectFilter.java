	...
	public static void parse(String file, String out) throws IOException {
		...
		while (br.ready()) {
			...
			if (line.indexOf("_DIF") == -1) { // 不含DIF
				...
				if (lineArr[1].equalsIgnoreCase("005")
						|| lineArr[1].equalsIgnoreCase("001")
						|| lineArr[1].equalsIgnoreCase("003")) {// eg: WRSI_005
					fw.write(line + "\r\n");
					continue;
				}
			} else {// 有DIF
				...
				if (line.indexOf(".") >= 0) {// 含float，也就是純粹與數值相比的
					...
					if (line.split("_DIF_")[0].indexOf(".") >= 0) {
						if (line.split("_DIF_")[1].indexOf("005") != -1
								|| line.split("_DIF_")[1].indexOf("003") != -1
								|| line.split("_DIF_")[1].indexOf("001") != -1) {
							fw.write(line + "\r\n");
							continue;
						}
					} else {
					...
					case 5:
						if (line.split("_DIF_")[0].split("_")[1]
								.equalsIgnoreCase("005") || line.split("_DIF_")[0].split("_")[1]
										.equalsIgnoreCase("003") || line.split("_DIF_")[0].split("_")[1]
